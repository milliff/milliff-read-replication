...
modelsummary(list(primary_school_model, primary_school_model2), coef_map = c("pc_school" = "Schools per Capita",
                                                                                           "pc_school_novb" = "Non-VB Schools per Capita"),
                           stars = T, output = "./results/tabs/TabA15.tex")
...
tm_shape(cfir_map_st) +tm_polygons(
  fill = "cfir_pc",
  fill.scale = tm_scale("cfir_pc"),
  values = "reds",
  values.range = c(.15,1),
  fill.legend = tm_legend("% Counter-FIR"),
  label.na = "No Hate Crimes",
) + tm_title("Prevalence of Counter-FIR Filing in States/UTs with Hate Crimes") |> 
  tmap_save(filename = "./results/figs/FigA4.png")

tmap_mode("plot")
tm_shape(cfir_map) + tm_polygons(
  fill = "cfir_pc",
  fill.scale = tm_scale("cfir_pc"),
  fill.legend = tm_legend("% Counter-FIR"),
  values = "reds",
  label.na = "No Hate Crimes"
) + tm_title("Prevalence of Counter-FIR Filing in Districts with Hate Crimes")|> 
  tmap_save(filename = "./results/figs/counter_fir_district_map.png")

counter_mod_shr <- lm_robust(I(counterFIR/count) ~ vb_count + tot_pop + pc_sc + pc_st + pc_illt + pc_male + 
...
counter_mod_ff <- lm_robust(I(counterFIR/count) ~ vb_count + tot_pop + pc_sc + pc_st + pc_illt + pc_male + 
                              wf_male + pc_muslim + pc_christian + prim_school + tar_road + 
                              dirt_road +  tdist_100k + tdist_500k + nightlight_08 + 
                              mean_time_decision + vio_crim + final_founding, data = df, clusters = district, fixed_effects = state_ut) 
counter_bjp_14 <- lm_robust(I(counterFIR/count) ~ BJP_share_14 + vb_count + tot_pop + pc_sc + pc_st + pc_illt + pc_male + 
                           wf_male + pc_muslim + pc_christian + prim_school + tar_road + 
                           dirt_road +  tdist_100k + tdist_500k + nightlight_08 + 
                           mean_time_decision + vio_crim, clusters = district, data = df)
counter_bjp_gov <- lm_robust(I(counterFIR/count) ~ BJP_gov_17 + vb_count + tot_pop + pc_sc + pc_st + pc_illt + pc_male + 
                              wf_male + pc_muslim + pc_christian + prim_school + tar_road + 
                              dirt_road +  tdist_100k + tdist_500k + nightlight_08 + 
                              mean_time_decision + vio_crim, clusters = district, data = df)

modelsummary::modelsummary(list("Counter FIR (All)" = counter_mod_shr, "Counter FIR (IM)" = counter_mod_ff, "Counter FIR (BJP)" = counter_bjp_14, "Counter FIR (Gov)" = counter_bjp_gov),
                           ...
                           add_rows = data.frame("State FEs?", "Y", "Y", "N", "N"),
                           stars = T, output = "./results/tabs/TabA17.tex")
...
modelsummary::modelsummary(list("BJP Vote Share 2014 (Bivariate)" = bjp_hc, "BJP in State Gov. (Bivariate)" = bjp_gov,
                           ...
                           coef_map = c("BJP_share_14" = "2014 BJP Voteshare (State)",
                                           ...
                                           "vio_crim" = "IPC Violent Crime Count",
                                        "(Intercept)"= "(Intercept)"), 
                           stars = T, output = "./results/tabs/TabA18.tex")
...
modelsummary::modelsummary(list("Post 2009 VB Construction" = placebo09, "Post 2014 VB Construction" = placebo14),
                           ...
                           output = "./results/tabs/TabA16.tex")
...
modelsummary::modelsummary(list("HC Count" = muslim_biv, "HC Count" = christian_biv, "HC Count" = count_rel_change, "Fatal HC Count" = fatal_rel_change, "Counter-FIR Count" = counter_rel_change), stars = T, title = "Correlation between VB Schools and Attempted Lynching Count, Adjusting for Recent Change in Religious Demography",
                           ...
                           output = "./results/tabs/TabA19.tex")
